...
%DIF ADD mainos.tex       Mon Feb 17 15:50:03 2020
%% ---------------------------------
...
The vertical mixing of tracers and momentum is done by a k-$\epsilon$ model (GLS, \citet{umlauf2003}). The effect of bottom friction is parameterized through a logarithmic law of the wall with a roughness length $Z_{0} = 0.01$ m. \DIFaddbegin \DIFadd{We use no explicit viscosity or diffusivity as the nemerical scheme induces sufficient numerical diffusion.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{The barotropic vorticity equation is obtained by integrating }\DIFdelend \DIFaddbegin \DIFadd{Two barotropic vorticity equations can be obtained depending on the choice of vertically integrating or averaging }\DIFaddend the momentum equations \DIFdelbegin \DIFdel{in the vertical and }\DIFdelend \DIFaddbegin \DIFadd{before }\DIFaddend cross-differentiating \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{them. The vertical averaging method allows us to define the main components driving the flow across f/h contours. One the other hand, the vertically integrated momentum equations give more informations on what is driving the barotropic flow. This last definition will be used in the following and can be written as }\DIFaddend \citep{gula2015}:
...
The difference between the two balances is highlighted by looking at the balance in the region in-between the two contours, which covers the upper slope and the shelf. It corresponds to a balance between BPT, NL and bottom drag. \DIFaddbegin \DIFadd{The NL term is only significant around Greenland shelf and is related to eddies interactions between shelf and open ocean. Otherwise, the vorticity is negative on the shelf thus explaining the positive sign of the bottom drag. The main source of anticyclonic vorticity is related to the BPT. }\DIFaddend This balance is close to the one described in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{csanady1978} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{csanady1978,csanady1997} }%DIFAUXCMD
}\DIFaddend and evokes a buoyancy driven flow in this area \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
We can identify several processes providing cyclonic barotropic vorticity to the subpolar gyre. The most important is the eddy contribution coming from the boundary area that is associated with a barotropic contribution. \DIFdelbegin \DIFdel{Barotropic vorticity is also }\DIFdelend \DIFaddbegin \DIFadd{Another contribution of barotropic vorticity in our gyre interior is }\DIFaddend provided through a mean-baroclinic signal coming from the NWC. \DIFaddbegin \DIFadd{Even though the NWC seems disconnected from the gyre interior and might not influence it directly. Its presence is essential to prevent water masses drift which might indirectly impact the gyre dynamics. }\DIFaddend In comparison, in the lower resolution simulation (not shown) most of the vorticity is advected inside the gyre by mean-barotropic processes but the amplitude of the NL term is cut by half.
